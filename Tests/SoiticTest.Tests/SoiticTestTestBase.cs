...
namespace SoiticTest.Tests
{
    public abstract class SoiticTestTestBase : AbpIntegratedTestBase<SoiticTestTestModule>
    {
        ...
        protected void UsingDbContext(Action<SoiticTestDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SoiticTestDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SoiticTestDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SoiticTestDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SoiticTestDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SoiticTestDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SoiticTestDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SoiticTestDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SoiticTestDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SoiticTestDbContext, Task<T>> func)
        {
            T result;
